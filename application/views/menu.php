...
?>
	<?php
		...
		foreach($category as $categorylist)
		{
			...
			foreach($categorylist['data'] as $subcat)
			{
				echo '<ul> <li><h3><a href="'.base_url().'index.php/controller/function/'.$subcat['id'].'">'.$subcat['name'].'</a></h3></li>';//echo $subcat['id'];//echo $subcat['name'];
				
				foreach($subcat['data'] as $originals)
				{
					echo '<li><a href="'.base_url().'index.php/controller/function/'.$originals['id'].'">'.$originals['name'].'</a></li>';// echo $originals['id'];// echo $originals['name'];
				}
				echo "</ul>";
